...
const dictfile = ("~/src/nushell-config/src/genpass-dict-jp" | path expand)
...
def "nu-complete variants" [] { [regular mixnmatch alphanum alpha diceware] }

#======= NUPASS PASSWORD GENERATOR =======
# Generate password of 3 dictionary file words, numbers and symbols
export def main [
  words: int = 5                                             # Number of words in password
  --word_length (-l): int = 8                                # Max length of words in password
  --symbols (-s): string = "!@#$%^&()_-+[]{}"                # Symbols to use in password
  --variant (-v): string@"nu-complete variants" = "diceware" # Password style to generate
  --delimiter (-m): string = "-"                             # Delimiter for diceware variant
