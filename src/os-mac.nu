...
export def main [--no-restart] {
    # Warn that some commands will not be run if the script is not run as root.
    if !is-admin {
        print $"(ansi pb)Certain commands will not be run without sudo privileges(ansi reset)"
        sleep 1sec
    }
    ...
    if $no_restart {
        for app in [cfprefsd Dock Finder Mail SystemUIServer Terminal] {
            killall $app
        }
    }

    print $"(ansi pb)Please log out and log back in to make all settings take effect(ansi reset)"
